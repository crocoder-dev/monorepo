...
<NonHSubtitle>Output</NonHSubtitle>

```javascript
Copy code
[ { name: 'Alice', average: 89 }, { name: 'Charlie', average: 90 }, { name: 'David', average: 100 } ]
...
[ { category: 'Clothes', average: 62.5 }, { category: 'Electronics', average: 70 } ]

```

In this example, we first use **`reduce`** to group the products by category, we then use **`map`** to iterate over the categories, after which use **`reduce`** again to sum all of the prices and divide by the number of products in that category to get the average price.
Then we use filter to only include categories with an average price above a certain threshold.
