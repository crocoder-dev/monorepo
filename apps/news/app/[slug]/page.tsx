...
export default async function Editions(context: { params: { slug: string } }) {
  ...
  const editionWithPosts = await db.select({ title: editions.title, date: editions.date, slug: editions.slug, description: editions.description, abstract: editions.abstract, post: databasePosts}).from(editions).innerJoin(databasePosts, eq(editions.id, databasePosts.editionId)).where(eq(editions.slug, slug));
