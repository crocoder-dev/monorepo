...
export default async function Home() {
  ...
  const editionWithPosts = await db.select({title: sql<string>`lastEdition.title`, slug: sql<string>`lastEdition.slug`, description: sql<string>`lastEdition.description`, abstract: sql<string>`lastEdition.abstract`, date: sql<Date>`lastEdition.date`, post: databasePosts}).from(lastEdition).innerJoin(databasePosts, eq(lastEdition.id, databasePosts.editionId)).orderBy(asc(databasePosts.order));
  ...
  const {title, date, abstract, description, slug} = editionWithPosts[0]!;

  return (
    <Posts posts={posts} edition={{title, date: new Date(date), abstract, slug, description}}/>
  )
}