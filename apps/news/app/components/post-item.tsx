      ...
      <div className="w-full flex-1 dark:text-white">
        ...
        <div className="px-4">
          {summary.split('\n\n').map((summary: string, i: number)=> {
            return <p key={id + '-' + i} className="py-4">{i === 0 ? (<span>📰<span className="font-semibold underline">TL;DR -</span> </span>) : null}<span className='text-gray-800 dark:text-gray-100'>{summary}</span></p>
          })}
          {insight ? <p className="py-4">💡<span className="font-semibold underline">Insight -</span> <span className='text-gray-800 dark:text-gray-100'>{insight}</span></p> : null}
