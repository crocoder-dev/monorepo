import blogHeader from "../../content/images/blogs/node-docker-image.png?preset=responsive";
...
export const meta = {
  ...
  title:
    "Dockerfile for BLAZINGLY FAST Dockerized Node.js apps",
  description:
    "Learn how to create perfomant and secure docker image and avoid issues with signals when running Node.js applications with npm scripts in this article. Discover how using a smaller Alpine-based image can reduce attack surface and why it's recommended to use execform notation in the Dockerfile's CMD directive.",
  category: "Docker",
  image: blogHeader,
  date: 1678659672751,
  ...
  abstract: "Learn how to set an explicit Docker base image tag and avoid issues with signals when running Node.js applications with npm scripts in this article. Discover how using a smaller Alpine-based image can reduce attack surface and why it's recommended to use execform notation in the Dockerfile's CMD directive.",
...
In this article, we'll take you through some best practices for creating a Dockerfile that can make your Node.js app run faster, more securely, and more efficiently. We'll cover topics like setting the Docker base image tag, avoiding npm scripts, properly handling signals, and running containers as a non-root user.
...
I personally suggest using the latest LTS or current version of Node.js pinned to the minor version (as of March 13th that would be 18.15 or 19.7, but check out node at [hub.docker](https://hub.docker.com/_/node)). This way you will always get the latest security patches and bug fixes, but still have a consistent build.
...
If you are certian that you don't need to know how signals work, you can skip to [this section](/#the-better-cmd-option) to get the updated dockerfile.
...
So let's put together everything we have disscussed so far, and create a `Dockerfile` that will run our Node.js app.
...
When you run the docker build command, Docker first checks if any of the previously built layers can be reused from cache. This saves time and speeds up the overall build process.

In the current Dockerfile, the COPY command copies everything in the current directory to the container's working directory. This means that all the application source code, as well as the package.json file, are copied. If you make any changes to your source code and run docker build again, Docker will have to rebuild the entire image from scratch, including running npm install. This can take a long time and slow down your development process.
...
Although it may appear redundant, specifying this environment variable is necessary.
...
Furthermore, we want to install only production dependencies, but we don't want to update package-lock.json file. In this case we can use `npm ci` command instead of `npm install`.
...
Let other Dockerized Node.js app users know which port the application expects to be listening on by including that information in the Dockerfile. You'll still need to make sure the port is available when the application runs, but this will help users understand what to expect!
...
However, there are some cases where Docker can't use layer caching, such as when a file is changed in a later step. This means Docker has to recreate all the layers after the change, even if they haven't actually been modified. This can slow down the build process.
...
Lets update our Dockerfile to use cache mount for npm dependencies.
...
Multi-stage environments in Docker are a way to make images smaller and faster. 
...
Let take a look at updated multi-stage Dockerfile and then we can discuss what's going on.
...
## Copy-pastable Improved Blazingly fast Dockerfile

Here you, go copy-paste this Dockerfile and you are good to go.
