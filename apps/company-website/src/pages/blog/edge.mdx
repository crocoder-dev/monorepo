...
Most of the cold starts add about 50 to 500ms overhead, but there are cases where cold starts can get up to 2 to 3 sec. For example, hitting cold start while connecting to a db via prisma ORM in a node.js serverless function [https://github.com/prisma/prisma/issues/10724](took over 2.5 seconds). [https://www.prisma.io/blog/prisma-and-serverless-73hbgKnZ6t](Prisma team have solved the issue) while I was writing the article, so better bump your Prisma verison.
