...
Not so long ago Edge functions were presented; a serverless function that run at the Edge. Edge functions differ a bit from serverless functions, usually they run on a restricted, sandboxed runtime, physically closer to the user, distrubuted through all the edge centers and without any cold start.

So before we start blindly moving your serverless functions to the edge, let's take a deeper look at how we can use edge functions.
