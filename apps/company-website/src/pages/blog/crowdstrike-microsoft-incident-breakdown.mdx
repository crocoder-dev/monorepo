...
export const meta = {
  ...
  description:
    'CrowdStrike\'s Falcon Sensor update caused "blue screens of death" on millions of Windows systems.',
...
At the first look it seems there is an error related to [NULL pointer dereference](https://cwe.mitre.org/data/definitions/476.html) in the C++ code. But at further decompiling of the code you can see the NULL check before the dereference. THe rpogram was reading pointers from a table in a lopp, and some of these pointers were invalid. Probably this is an error in parsing of a configuration file, what left [https://cwe.mitre.org/data/definitions/824.html](some entries uninitialized).
