...
Design a queue system that accepts promise factories (functions that return promises) and manages their execution. The queue should execute these promise factories, but must limit the number of concurrently running tasks to three. When one task completes, the next task in the queue should start.
...
Ensure that no more than three tasks are running at any given time. When a task completes, the next task in the queue (if any) should start.
