...
It should light up the mood a bit, because this article isn't about JavaScript quirks. It's a about objectivly worst and most annoying features of the language.

I have encountered uncountable amount of bugs written in the code because of these keywords. My hair gotten gray when I needed to support Internet Exlorer 7, where all the modern JavaScript keywords aren't availiable.

Back in 2007 we didn't knew better. We have used the tools that were availiable. As JavaScript developers we had a saying back in the time: *"At least our company doesn't use [Silverlight](https://en.wikipedia.org/wiki/Microsoft_Silverlight)."*
...
I mean, back in the days we only had `var` at our disposal. We were using it, but we weren't happy.

Today in modern JavaScript we have `let` and `const`. Please use them over `var`.

## eval

`eval` is pure evil. I don't want to elaborte. Here is a non-so fictional story about it.
...
The code, now a malevolent force, spread through the website's veins, injecting itself into every user session, every page, every corner of the once-peaceful digital realm. It morphed, replicated, and before long, it was not just the website that was haunted—the infection spread to the machines of every visitor, a digital plague born from a single line of eval.
...
In the aftermath, the web whispered tales of the cursed site, a cautionary lore passed among developers. The eval function became a symbol of hubris, a relic of the past that served as a warning: some shortcuts lead not to glory, but to the haunted depths of a nightmare.
...
`with` is a pandora's box that I don't want to revisit. I am not sure whose idea this was, but this is one of the worst idea after the "billion dollar mistake `null`. It's deprecated for a really good reason.

In the time bofore Google, developers wanted to deconstruct an object. So they used with.
